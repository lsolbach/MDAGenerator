...
<!--
	...
	TODO: convert '.' to '_' in element names (types)
	-->
...
import org.soulspace.xmi.marshal.<xsl:value-of select="translate(../../@name, '.', '_')"/>;
	...
	<xsl:template
		match="xs:element[@name and not(starts-with(@name, 'XMI'))]/xs:complexType/xs:attribute[@ref='xmi.id']"
		mode="collections">
	List&lt;<xsl:value-of select="translate(../../@name, '.', '_')"/>&gt; xmi<xsl:value-of select="translate(../../@name, '.', '_')"/>List = new ArrayList&lt;<xsl:value-of select="translate(../../@name, '.', '_')"/>&gt;();
	</xsl:template>
